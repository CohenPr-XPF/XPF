...
class AlphabetToIpa {
    ...
    translate(source) {
	// Check if in words list (could be used to store words that were already translated too)
  	if (source in this.words) {
	    ...
	    // now also lowercase it
  	    source = source.toLowerCase()

	    // get ready for letter by letter translation
		...
		//
		// Match rules get a first go 
		//
		if (sfrom in this.matches) {
                    target_list.push(this.matches[sfrom]);  
		} else { // otherwise we're looking for sub rules
